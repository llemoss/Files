...
local function offscreenBalloon(self, event)
	...
	if (self.y > display.contentHeight*1.5) then
		...
		self:removeSelf()
		if score > 0 and (level == "easy" or level == "hard") then
			balloonsMissed = balloonsMissed + 1
			missedText.text = string.format(missedTranslated[language] .. " %d/10", balloonsMissed)
		elseif score > 0 and level == "timed" then
			balloonsMissed = balloonsMissed + 1
			seconds = seconds - 1
			timeText.text = string.format(timeTranslated[language] .. " %d", seconds)
		else
...
local function addNewBallonOrBomb()
	startXballoon = math.random(display.contentWidth*0.2, display.contentWidth*0.85)
	startXbomb = math.random(display.contentWidth*0.2-20, display.contentWidth*0.85-20)
	if (math.random(1, bombChance) == 1) then
		startXballoon = math.random(display.contentWidth*0.2, display.contentWidth*0.85)
		bomb = display.newImage("bombzero.png", startXbomb, startY+200, true)
		print("Red bomb:", bomb.x)
	...
	elseif (math.random(1, 150) == 1) then
		startXballoon = math.random(display.contentWidth*0.2, display.contentWidth*0.85)
		bomb = display.newImage("bomb.png", startXbomb, -300, true)
		physics.addBody(bomb, {radius = 50, friction = 0, bounce = 0, isSensor=true})
		bomb.enterFrame = offscreen
		Runtime:addEventListener("enterFrame", bomb)
		bomb:addEventListener("touch", bombzeroTouched)
		objectsCreated = objectsCreated + 1
	elseif (math.random(1, 90) == 1) then
		startXballoon = math.random(display.contentWidth*0.2, display.contentWidth*0.85)
		balloon = display.newImage("blue_balloon.png", startXballoon, -700, true)
		...
		balloon:addEventListener("touch", blueballoonTouched)
		objectsCreated = objectsCreated + 1
	elseif (math.random(1, 80) == 1) then
		startXballoon = math.random(display.contentWidth*0.2, display.contentWidth*0.85)
		balloon = display.newImage("purple_balloon.png", startXballoon, -500, true)
		physics.addBody(balloon, {radius = 80, friction = 0, bounce = 0, isSensor=true})
		balloon.enterFrame = offscreen
		Runtime:addEventListener("enterFrame", balloon)
		balloon:addEventListener("touch", yellowballoonTouched)
		objectsCreated = objectsCreated + 1
	elseif (math.random(1, 190) == 1) then
		startXballoon = math.random(display.contentWidth*0.2, display.contentWidth*0.85)
		balloon = display.newImage("purple_balloon2.png", startXballoon, -700, true)
		physics.addBody(balloon, {radius = 80, friction = 0, bounce = 0, isSensor=true})
		balloon.enterFrame = offscreenBalloon
		Runtime:addEventListener("enterFrame", balloon)
		balloon:addEventListener("touch", purpleballoonTouched)
		objectsCreated = objectsCreated + 1
	else
		startXballoon = math.random(display.contentWidth*0.2, display.contentWidth*0.85)
		balloon = display.newImage(balloonImage, startXballoon + math.random(-20, 20), startY + math.random(-100, -75), true)
		print("Red balloon:", balloon.x)
		physics.addBody(balloon, {radius = 80, friction = 0, bounce = 0, isSensor=true})
		balloon.enterFrame = offscreenBalloon
		Runtime:addEventListener("enterFrame", balloon)
...
local function verif()
	...
	if (objectsCreated > multQuant and quantBalloons < maxBalloons) then
		delay = delay + delayBonus + math.random(50, 100)
...
local function game()
		...
		if muted == false then
			...
			audio.setMaxVolume(0.4, {channel=1})
		end
		...
		if level == "hard" or level == "easy" then
		...
		if (gameover == false) then
			addNewBallonOrBomb()
			verif()
			timereplay[0] = timer.performWithDelay(delay, addNewBallonOrBomb, 0)
...
local function gameoverVerif()
	...
	if (gameover == true and replayConfirm == true) then
		...
		if timesReplayed >= 1 then
			...
			ratingButton.x = display.contentWidth*0.5
			ratingButton.y = display.contentHeight - 125
			reBtn.x = display.contentWidth*0.5 + 65
			reBtn.y = display.contentHeight - 85
			scoreSubmitButton.x = display.contentWidth*0.5 - 65
		end
		ads.show( "interstitial", { appId = interstitialID, testMode = false} )
		gameSettings = {
...
local function initGame()
	...
	playBtn.x = display.contentWidth*0.5 - 60
	playBtn.y = display.contentHeight - 160
	levelBtn.x = display.contentWidth*0.5 + 65
	levelBtn.y = display.contentHeight - 160
	welcomeText.x = display.contentWidth*0.5
	welcomeText.y = display.contentHeight*0.5 - 30
	credBtn.x = display.contentWidth*0.5
	credBtn.y = display.contentHeight - 80
	muteBtn.x = display.contentWidth*0.5 - 60
	muteBtn.y = display.contentHeight - 120
	vibrateBtn.x = display.contentWidth*0.5 + 60
	vibrateBtn.y = display.contentHeight - 120
	touch = 1
end
